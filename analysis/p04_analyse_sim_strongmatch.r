## p04_analyse_sim_strongmatch.r
## dcmuller
##
## process and analyse simulation samples generated by p03_sim_strongmatch.r 
library(ggplot2)
library(devtools)
library(data.table)
library(reshape2)
...
params <- read.csv("./analysis/output/o03_params.csv", stringsAsFactors = FALSE)
params <- data.table(params)

## read data and rename
samples <- read.csv("./analysis/output/o03_all_samples.csv",
...
samples_long <- melt(samples, 
                     ...
                     measure.vars  = c("intercept", "exposure", "matchvar_centred", "log_scale"), 
                     value.name    = "estimate",
                     variable.name = "param")
...
tabcox <- summary_samples[(model == "coxfull" | 
                             model == "conditional" |
                             model == "coxweighted") 
                            & param == "exposure",
...
tabcox[["model"]] <- ifelse(tabcox[["model"]] == "coxfull", 
                            "Full cohort",
                            ifelse(tabcox[["model"]] == "coxweighted",
                                   "Weighted NCC",
                                   "Conditional"))
...
samples_weighted <- melt(samples_weighted, 
                         id.vars = c("settings", "model", "param_type", 
                                     ...
                                     "matchvar_centred", "exposure", "log_scale"),
                         value.name = "xb",
                         variable.name = "x")
...
pred_long <- melt(pred, 
                  id.vars    = c("settings", "model", "param_type", "sim_id", 
                                 "sim", "intercept", "exposure", "matchvar_centred",
                                 "log_scale", "x", "xb"), 
                  value.name = "time")
...
CairoPDF(file = "./analysis/output/g04_cumul_risk_rare_match.pdf",
         width = 6, 
         height = 5)
surv_plot
dev.off()
...
CairoPDF(file = "./analysis/output/g04_cumul_risk_common_match.pdf",
         width = 6, 
         height = 5)
surv_plot
dev.off()
